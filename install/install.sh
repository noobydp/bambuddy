...
#
# Usage:
#   Interactive:  curl -fsSL https://raw.githubusercontent.com/noobydp/bambuddy/main/install/install.sh -o install.sh && chmod +x install.sh && ./install.sh
...
download_bambuddy() {
    ...
    if ! git ls-remote --exit-code --heads https://github.com/noobydp/bambuddy.git "$BRANCH" &>/dev/null; then
        log_error "Branch '$BRANCH' not found in the BamBuddy repository."
        log_info "Available branches:"
        git ls-remote --heads https://github.com/noobydp/bambuddy.git | sed 's|.*refs/heads/|  - |'
        exit 1
    fi
    ...
    if [[ "$OS_TYPE" == "macos" ]]; then
        ...
        if [[ -d "$INSTALL_PATH/.git" ]]; then
            ...
            git clone --branch "$BRANCH" https://github.com/noobydp/bambuddy.git "$INSTALL_PATH"
        fi
        ...
        sudo git clone --branch "$BRANCH" https://github.com/noobydp/bambuddy.git "$INSTALL_PATH"
        sudo chown -R "$SERVICE_USER:$SERVICE_USER" "$INSTALL_PATH" 2>/dev/null || true
    fi
...
Documentation=https://github.com/noobydp/bambuddy
